import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { z } from "zod";
import * as api from "./api.js";
import * as tasksApi from "./tasks-api.js";
import * as driveApi from "./drive-api.js";
import * as sheetsApi from "./sheets-api.js";
import * as docsApi from "./docs-api.js";
import * as gmailApi from "./gmail-api.js";
import * as slidesApi from "./slides-api.js";
import { resolveAccountWithAcl, listAccountsWithAcl } from "./acl.js";
import { generateAuthUrl } from "./auth.js";
import { logger } from "../../config.js";

// ── Reusable account parameter ─────────────────────

const accountParam = z.string().optional().describe(
    "Account name to use (optional if only one account is connected)"
);

// ── Register Tools ───────────────────────────────────

export function registerGoogleWorkspaceTools(server: McpServer): void {
    // Tool: google-workspace-create-event
    server.tool(
        "google-workspace-create-event",
        "Create a new event in Google Workspace",
        {
            summary: z.string().describe("Event title"),
            description: z.string().optional().describe("Event description"),
            start_time: z.string().describe("Start time in ISO format (e.g., 2024-01-15T10:00:00Z)"),
            end_time: z.string().describe("End time in ISO format (e.g., 2024-01-15T11:00:00Z)"),
            attendees: z.array(z.string()).optional().describe("List of attendee email addresses"),
            location: z.string().optional().describe("Event location"),
            timezone: z.string().optional().describe("Timezone (e.g., America/New_York, Europe/Paris). Defaults to UTC"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const eventLink = await api.createEvent(accountName, {
                    summary: args.summary,
                    description: args.description,
                    start_time: args.start_time,
                    end_time: args.end_time,
                    attendees: args.attendees,
                    location: args.location,
                    timezone: args.timezone,
                });

                return {
                    content: [
                        {
                            type: "text",
                            text: `Event created successfully!\n\nTitle: ${args.summary}\nLink: ${eventLink}`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-workspace] create-event error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-workspace-list-events
    server.tool(
        "google-workspace-list-events",
        "List upcoming events from Google Workspace",
        {
            max_results: z.number().optional().describe("Maximum number of events to return (default: 10)"),
            time_min: z.string().optional().describe("Filter events starting after this time (ISO format)"),
            time_max: z.string().optional().describe("Filter events starting before this time (ISO format)"),
            query: z.string().optional().describe("Search query to filter events"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const events = await api.listEvents(accountName, {
                    max_results: args.max_results,
                    time_min: args.time_min,
                    time_max: args.time_max,
                    query: args.query,
                });

                if (events.length === 0) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: "No events found.",
                            },
                        ],
                    };
                }

                const eventList = events
                    .map((event, index) => {
                        const start = event.start?.dateTime || event.start?.date || "No start time";
                        const summary = event.summary || "No title";
                        const id = event.id || "No ID";
                        return `${index + 1}. ${summary}\n   Start: ${start}\n   ID: ${id}`;
                    })
                    .join("\n\n");

                return {
                    content: [
                        {
                            type: "text",
                            text: `Found ${events.length} event(s):\n\n${eventList}`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-workspace] list-events error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-workspace-get-event
    server.tool(
        "google-workspace-get-event",
        "Get details of a specific event from Google Workspace",
        {
            event_id: z.string().describe("The ID of the event to retrieve"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const event = await api.getEvent(accountName, args.event_id);

                const details = `Event Details:
Title: ${event.summary || "No title"}
Description: ${event.description || "No description"}
Location: ${event.location || "No location"}
Start: ${event.start?.dateTime || event.start?.date || "No start time"}
End: ${event.end?.dateTime || event.end?.date || "No end time"}
Attendees: ${event.attendees?.map((a: any) => a.email).join(", ") || "None"}
Link: ${event.htmlLink || "No link"}
ID: ${event.id || "No ID"}`;

                return {
                    content: [
                        {
                            type: "text",
                            text: details,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-workspace] get-event error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-workspace-update-event
    server.tool(
        "google-workspace-update-event",
        "Update an existing event in Google Workspace",
        {
            event_id: z.string().describe("The ID of the event to update"),
            summary: z.string().optional().describe("New event title"),
            description: z.string().optional().describe("New event description"),
            start_time: z.string().optional().describe("New start time in ISO format"),
            end_time: z.string().optional().describe("New end time in ISO format"),
            attendees: z.array(z.string()).optional().describe("New list of attendee email addresses"),
            location: z.string().optional().describe("New event location"),
            timezone: z.string().optional().describe("Timezone for the new times"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const eventLink = await api.updateEvent(accountName, {
                    event_id: args.event_id,
                    summary: args.summary,
                    description: args.description,
                    start_time: args.start_time,
                    end_time: args.end_time,
                    attendees: args.attendees,
                    location: args.location,
                    timezone: args.timezone,
                });

                return {
                    content: [
                        {
                            type: "text",
                            text: `Event updated successfully!\n\nLink: ${eventLink}`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-workspace] update-event error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-workspace-delete-event
    server.tool(
        "google-workspace-delete-event",
        "Delete an event from Google Workspace",
        {
            event_id: z.string().describe("The ID of the event to delete"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await api.deleteEvent(accountName, args.event_id);

                return {
                    content: [
                        {
                            type: "text",
                            text: result,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-workspace] delete-event error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-workspace-auth
    server.tool(
        "google-workspace-auth",
        "Start OAuth flow to connect a Google Workspace account",
        {
            account_name: z.string().describe("A name for this Google Workspace account (e.g., 'personal', 'work')"),
        },
        async (args, extra) => {
            try {
                const authUrl = generateAuthUrl(args.account_name);

                return {
                    content: [
                        {
                            type: "text",
                            text: `To connect your Google Workspace account "${args.account_name}", please visit:\n\n${authUrl}\n\nAfter authorizing, you'll be redirected to the callback URL and the account will be saved.`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-workspace] auth error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-workspace-list-accounts
    server.tool(
        "google-workspace-list-accounts",
        "List all connected Google Workspace accounts (filtered by your API key permissions)",
        {},
        async (_args, extra) => {
            try {
                const accounts = await listAccountsWithAcl(extra.sessionId);

                if (accounts.length === 0) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: "No Google Workspace accounts connected. Use google-workspace-auth to connect one.",
                            },
                        ],
                    };
                }

                return {
                    content: [
                        {
                            type: "text",
                            text: `Connected accounts: ${accounts.join(", ")}`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-workspace] list-accounts error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],

                };
            }
        }
    );

    // Tool: google-tasks-list-task-lists
    server.tool(
        "google-tasks-list-task-lists",
        "List all task lists from Google Tasks",
        {
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const taskLists = await tasksApi.listTaskLists(accountName);

                if (taskLists.length === 0) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: "No task lists found.",
                            },
                        ],
                    };
                }

                const list = taskLists
                    .map((tl, i) => `${i + 1}. ${tl.title || "Untitled"}\n   ID: ${tl.id}`)
                    .join("\n\n");

                return {
                    content: [
                        {
                            type: "text",
                            text: `Found ${taskLists.length} task list(s):\n\n${list}`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-tasks] list-task-lists error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-tasks-create-task-list
    server.tool(
        "google-tasks-create-task-list",
        "Create a new task list in Google Tasks",
        {
            title: z.string().describe("Title of the new task list"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const taskList = await tasksApi.createTaskList(accountName, args.title);

                return {
                    content: [
                        {
                            type: "text",
                            text: `Task list created successfully!\n\nTitle: ${taskList.title}\nID: ${taskList.id}`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-tasks] create-task-list error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-tasks-list-tasks
    server.tool(
        "google-tasks-list-tasks",
        "List tasks from a Google Tasks list",
        {
            task_list_id: z.string().optional().describe("Task list ID (defaults to primary list '@default')"),
            max_results: z.number().optional().describe("Maximum number of tasks to return (default: 100)"),
            show_completed: z.boolean().optional().describe("Whether to show completed tasks (default: true)"),
            due_min: z.string().optional().describe("Filter tasks due after this date (RFC 3339 format)"),
            due_max: z.string().optional().describe("Filter tasks due before this date (RFC 3339 format)"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const tasks = await tasksApi.listTasks(accountName, {
                    task_list_id: args.task_list_id,
                    max_results: args.max_results,
                    show_completed: args.show_completed,
                    due_min: args.due_min,
                    due_max: args.due_max,
                });

                if (tasks.length === 0) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: "No tasks found.",
                            },
                        ],
                    };
                }

                const taskList = tasks
                    .map((t, i) => {
                        const status = t.status === "completed" ? "[x]" : "[ ]";
                        const due = t.due ? `\n   Due: ${t.due}` : "";
                        const notes = t.notes ? `\n   Notes: ${t.notes}` : "";
                        return `${i + 1}. ${status} ${t.title || "No title"}${due}${notes}\n   ID: ${t.id}`;
                    })
                    .join("\n\n");

                return {
                    content: [
                        {
                            type: "text",
                            text: `Found ${tasks.length} task(s):\n\n${taskList}`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-tasks] list-tasks error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-tasks-get-task
    server.tool(
        "google-tasks-get-task",
        "Get details of a specific task from Google Tasks",
        {
            task_id: z.string().describe("The ID of the task to retrieve"),
            task_list_id: z.string().optional().describe("Task list ID (defaults to '@default')"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const task = await tasksApi.getTask(accountName, args.task_list_id || "@default", args.task_id);

                const details = `Task Details:
                Title: ${task.title || "No title"}
                Notes: ${task.notes || "No notes"}
                Status: ${task.status || "Unknown"}
                Due: ${task.due || "No due date"}
                Completed: ${task.completed || "Not completed"}
                Updated: ${task.updated || "Unknown"}
                Link: ${task.webViewLink || "No link"}
                ID: ${task.id || "No ID"}`;

                return {
                    content: [
                        {
                            type: "text",
                            text: details,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-tasks] get-task error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-tasks-create-task
    server.tool(
        "google-tasks-create-task",
        "Create a new task in Google Tasks",
        {
            title: z.string().describe("Task title (max 1024 characters)"),
            notes: z.string().optional().describe("Task notes/description (max 8192 characters)"),
            due: z.string().optional().describe("Due date in RFC 3339 format (e.g., 2024-01-15T00:00:00Z). Only date part is used."),
            task_list_id: z.string().optional().describe("Task list ID (defaults to '@default')"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const task = await tasksApi.createTask(accountName, {
                    title: args.title,
                    notes: args.notes,
                    due: args.due,
                    task_list_id: args.task_list_id,
                });

                return {
                    content: [
                        {
                            type: "text",
                            text: `Task created successfully!\n\nTitle: ${task.title}\nID: ${task.id}${task.due ? `\nDue: ${task.due}` : ""}${task.webViewLink ? `\nLink: ${task.webViewLink}` : ""}`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-tasks] create-task error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-tasks-update-task
    server.tool(
        "google-tasks-update-task",
        "Update an existing task in Google Tasks",
        {
            task_id: z.string().describe("The ID of the task to update"),
            title: z.string().optional().describe("New task title"),
            notes: z.string().optional().describe("New task notes"),
            due: z.string().optional().describe("New due date in RFC 3339 format"),
            status: z.enum(["needsAction", "completed"]).optional().describe("New task status"),
            task_list_id: z.string().optional().describe("Task list ID (defaults to '@default')"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const task = await tasksApi.updateTask(accountName, {
                    task_id: args.task_id,
                    title: args.title,
                    notes: args.notes,
                    due: args.due,
                    status: args.status,
                    task_list_id: args.task_list_id,
                });

                return {
                    content: [
                        {
                            type: "text",
                            text: `Task updated successfully!\n\nTitle: ${task.title}\nStatus: ${task.status}\nID: ${task.id}`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-tasks] update-task error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-tasks-delete-task
    server.tool(
        "google-tasks-delete-task",
        "Delete a task from Google Tasks",
        {
            task_id: z.string().describe("The ID of the task to delete"),
            task_list_id: z.string().optional().describe("Task list ID (defaults to '@default')"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await tasksApi.deleteTask(accountName, args.task_list_id || "@default", args.task_id);

                return {
                    content: [
                        {
                            type: "text",
                            text: result,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-tasks] delete-task error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-tasks-complete-task
    server.tool(
        "google-tasks-complete-task",
        "Mark a task as completed in Google Tasks",
        {
            task_id: z.string().describe("The ID of the task to complete"),
            task_list_id: z.string().optional().describe("Task list ID (defaults to '@default')"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const task = await tasksApi.completeTask(accountName, args.task_list_id || "@default", args.task_id);

                return {
                    content: [
                        {
                            type: "text",
                            text: `Task completed!\n\nTitle: ${task.title}\nStatus: ${task.status}\nID: ${task.id}`,
                        },
                    ],
                };
            } catch (error) {
                logger.error("[google-tasks] complete-task error:", error);
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
            }
        }
    );

    // ── Drive Tools ─────────────────────────────────

    // Tool: google-drive-search
    server.tool(
        "google-drive-search",
        "Search for files in Google Drive",
        {
            query: z.string().describe("Search query (file name or content)"),
            page_size: z.number().optional().describe("Number of results per page (default: 10, max: 100)"),
            page_token: z.string().optional().describe("Token for the next page of results"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await driveApi.searchFiles(accountName, {
                    query: args.query,
                    page_size: args.page_size,
                    page_token: args.page_token,
                });

                if (result.files.length === 0) {
                    return { content: [{ type: "text", text: "No files found." }] };
                }

                const fileList = result.files
                    .map((f, i) => `${i + 1}. ${f.name}\n   Type: ${f.mimeType}\n   ID: ${f.id}\n   Modified: ${f.modifiedTime}${f.webViewLink ? `\n   Link: ${f.webViewLink}` : ""}`)
                    .join("\n\n");

                let text = `Found ${result.files.length} file(s):\n\n${fileList}`;
                if (result.nextPageToken) {
                    text += `\n\nMore results available. Use page_token: ${result.nextPageToken}`;
                }

                return { content: [{ type: "text", text }] };
            } catch (error) {
                logger.error("[google-drive] search error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-drive-read-file
    server.tool(
        "google-drive-read-file",
        "Read file content from Google Drive. Converts Workspace files: Docs to markdown, Sheets to CSV, Slides to plain text",
        {
            file_id: z.string().describe("The ID of the file to read"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await driveApi.readFile(accountName, args.file_id);

                return {
                    content: [{
                        type: "text",
                        text: `File: ${result.name}\nType: ${result.mimeType}\n\n${result.content}`,
                    }],
                };
            } catch (error) {
                logger.error("[google-drive] read-file error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-drive-list-folder
    server.tool(
        "google-drive-list-folder",
        "List contents of a Google Drive folder",
        {
            folder_id: z.string().optional().describe("Folder ID to list (defaults to root)"),
            page_size: z.number().optional().describe("Number of results per page (default: 20)"),
            page_token: z.string().optional().describe("Token for the next page of results"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await driveApi.listFolderContents(accountName, {
                    folder_id: args.folder_id,
                    page_size: args.page_size,
                    page_token: args.page_token,
                });

                if (result.files.length === 0) {
                    return { content: [{ type: "text", text: "Folder is empty." }] };
                }

                const fileList = result.files
                    .map((f, i) => {
                        const icon = f.mimeType === "application/vnd.google-apps.folder" ? "[Folder]" : "[File]";
                        return `${i + 1}. ${icon} ${f.name}\n   Type: ${f.mimeType}\n   ID: ${f.id}`;
                    })
                    .join("\n\n");

                let text = `Found ${result.files.length} item(s):\n\n${fileList}`;
                if (result.nextPageToken) {
                    text += `\n\nMore results available. Use page_token: ${result.nextPageToken}`;
                }

                return { content: [{ type: "text", text }] };
            } catch (error) {
                logger.error("[google-drive] list-folder error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-drive-rename
    server.tool(
        "google-drive-rename",
        "Rename a file or folder in Google Drive",
        {
            file_id: z.string().describe("ID of the file or folder to rename"),
            new_name: z.string().describe("New name for the file or folder"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await driveApi.renameFile(accountName, {
                    file_id: args.file_id,
                    new_name: args.new_name,
                });

                return {
                    content: [{
                        type: "text",
                        text: `File renamed successfully.\nName: ${result.name}\nID: ${result.id}\nLink: ${result.webViewLink || "N/A"}`,
                    }],
                };
            } catch (error) {
                logger.error("[google-drive] rename error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-drive-move
    server.tool(
        "google-drive-move",
        "Move a file or folder to a different folder in Google Drive",
        {
            file_id: z.string().describe("ID of the file or folder to move"),
            destination_folder_id: z.string().describe("ID of the destination folder"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await driveApi.moveFile(accountName, {
                    file_id: args.file_id,
                    destination_folder_id: args.destination_folder_id,
                });

                return {
                    content: [{
                        type: "text",
                        text: `File moved successfully.\nName: ${result.name}\nID: ${result.id}\nNew parent(s): ${(result.parents || []).join(", ")}\nLink: ${result.webViewLink || "N/A"}`,
                    }],
                };
            } catch (error) {
                logger.error("[google-drive] move error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-drive-trash
    server.tool(
        "google-drive-trash",
        "Move a file or folder to trash in Google Drive",
        {
            file_id: z.string().describe("ID of the file or folder to trash"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await driveApi.trashFile(accountName, args.file_id);

                return { content: [{ type: "text", text: result }] };
            } catch (error) {
                logger.error("[google-drive] trash error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-drive-create-folder
    server.tool(
        "google-drive-create-folder",
        "Create a new folder in Google Drive",
        {
            name: z.string().describe("Name of the new folder"),
            parent_folder_id: z.string().optional().describe("ID of the parent folder (defaults to root)"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await driveApi.createFolder(accountName, args.name, args.parent_folder_id);

                return {
                    content: [{
                        type: "text",
                        text: `Folder created successfully.\nName: ${result.name}\nID: ${result.id}\nLink: ${result.webViewLink || "N/A"}`,
                    }],
                };
            } catch (error) {
                logger.error("[google-drive] create-folder error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // ── Sheets Tools ────────────────────────────────

    // Tool: google-sheets-read
    server.tool(
        "google-sheets-read",
        "Read data from a Google Spreadsheet",
        {
            spreadsheet_id: z.string().describe("The ID of the spreadsheet to read"),
            ranges: z.array(z.string()).optional().describe("Array of A1 notation ranges (e.g., ['Sheet1!A1:B10']). Reads entire sheet if omitted."),
            sheet_id: z.number().optional().describe("Specific sheet ID to read (alternative to ranges)"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const data = await sheetsApi.readSheet(accountName, {
                    spreadsheet_id: args.spreadsheet_id,
                    ranges: args.ranges,
                    sheet_id: args.sheet_id,
                });

                const formatted = data
                    .map((vr) => {
                        const range = vr.range || "Sheet";
                        const rows = vr.values || [];
                        if (rows.length === 0) return `${range}: (empty)`;
                        return `${range}:\n${rows.map((r) => (r as string[]).join("\t")).join("\n")}`;
                    })
                    .join("\n\n");

                return { content: [{ type: "text", text: formatted }] };
            } catch (error) {
                logger.error("[google-sheets] read error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-sheets-update-cell
    server.tool(
        "google-sheets-update-cell",
        "Update a cell value in a Google Spreadsheet",
        {
            spreadsheet_id: z.string().describe("The ID of the spreadsheet"),
            range: z.string().describe("Cell range in A1 notation (e.g., 'Sheet1!A1')"),
            value: z.string().describe("New cell value"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await sheetsApi.updateCell(accountName, {
                    spreadsheet_id: args.spreadsheet_id,
                    range: args.range,
                    value: args.value,
                });

                return { content: [{ type: "text", text: result }] };
            } catch (error) {
                logger.error("[google-sheets] update-cell error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-sheets-append-rows
    server.tool(
        "google-sheets-append-rows",
        "Append rows of data to a Google Spreadsheet",
        {
            spreadsheet_id: z.string().describe("The ID of the spreadsheet"),
            range: z.string().describe("Range in A1 notation for where to append (e.g., 'Sheet1!A:Z')"),
            values: z.array(z.array(z.string())).describe("Array of rows, each row is an array of cell values"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await sheetsApi.appendRows(accountName, {
                    spreadsheet_id: args.spreadsheet_id,
                    range: args.range,
                    values: args.values,
                });

                return { content: [{ type: "text", text: result }] };
            } catch (error) {
                logger.error("[google-sheets] append-rows error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-sheets-create
    server.tool(
        "google-sheets-create",
        "Create a new Google Spreadsheet",
        {
            title: z.string().describe("Title of the new spreadsheet"),
            sheet_titles: z.array(z.string()).optional().describe("Names of sheets to create (defaults to one sheet)"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await sheetsApi.createSpreadsheet(accountName, {
                    title: args.title,
                    sheet_titles: args.sheet_titles,
                });

                return {
                    content: [{
                        type: "text",
                        text: `Spreadsheet created!\n\nID: ${result.spreadsheetId}\nURL: ${result.spreadsheetUrl}`,
                    }],
                };
            } catch (error) {
                logger.error("[google-sheets] create error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // ── Docs Tools ──────────────────────────────────

    // Tool: google-docs-read
    server.tool(
        "google-docs-read",
        "Read the content of a Google Document",
        {
            document_id: z.string().describe("The ID of the Google Document to read"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await docsApi.readDocument(accountName, args.document_id);

                return {
                    content: [{
                        type: "text",
                        text: `Document: ${result.title}\n\n${result.content}`,
                    }],
                };
            } catch (error) {
                logger.error("[google-docs] read error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-docs-create
    server.tool(
        "google-docs-create",
        "Create a new Google Document",
        {
            title: z.string().describe("Title of the new document"),
            body: z.string().optional().describe("Initial text content for the document"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await docsApi.createDocument(accountName, args.title, args.body);

                return {
                    content: [{
                        type: "text",
                        text: `Document created!\n\nID: ${result.documentId}\nURL: ${result.documentUrl}`,
                    }],
                };
            } catch (error) {
                logger.error("[google-docs] create error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // ── Gmail Tools ─────────────────────────────────

    // Tool: google-gmail-list-messages
    server.tool(
        "google-gmail-list-messages",
        "List recent emails from Gmail with optional search query",
        {
            query: z.string().optional().describe("Gmail search query (e.g., 'from:user@example.com is:unread', 'subject:invoice')"),
            max_results: z.number().optional().describe("Maximum number of messages to return (default: 10)"),
            label_ids: z.array(z.string()).optional().describe("Filter by label IDs (e.g., ['INBOX', 'UNREAD'])"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const messages = await gmailApi.listMessages(accountName, {
                    query: args.query,
                    max_results: args.max_results,
                    label_ids: args.label_ids,
                });

                if (messages.length === 0) {
                    return { content: [{ type: "text", text: "No messages found." }] };
                }

                const list = messages
                    .map((m, i) => `${i + 1}. ${m.subject}\n   From: ${m.from}\n   Date: ${m.date}\n   Snippet: ${m.snippet}\n   ID: ${m.id}`)
                    .join("\n\n");

                return {
                    content: [{ type: "text", text: `Found ${messages.length} message(s):\n\n${list}` }],
                };
            } catch (error) {
                logger.error("[google-gmail] list-messages error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-gmail-read-message
    server.tool(
        "google-gmail-read-message",
        "Read the full content of a specific Gmail message",
        {
            message_id: z.string().describe("The ID of the message to read"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const message = await gmailApi.readMessage(accountName, args.message_id);

                const details = `From: ${message.headers.from}
To: ${message.headers.to}${message.headers.cc ? `\nCc: ${message.headers.cc}` : ""}
Subject: ${message.headers.subject}
Date: ${message.headers.date}
Labels: ${message.labelIds.join(", ")}

${message.body}`;

                return { content: [{ type: "text", text: details }] };
            } catch (error) {
                logger.error("[google-gmail] read-message error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-gmail-send
    server.tool(
        "google-gmail-send",
        "Send an email via Gmail",
        {
            to: z.string().describe("Recipient email address"),
            subject: z.string().describe("Email subject"),
            body: z.string().describe("Email body (plain text)"),
            cc: z.string().optional().describe("CC recipient(s)"),
            bcc: z.string().optional().describe("BCC recipient(s)"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const messageId = await gmailApi.sendEmail(accountName, {
                    to: args.to,
                    subject: args.subject,
                    body: args.body,
                    cc: args.cc,
                    bcc: args.bcc,
                });

                return {
                    content: [{
                        type: "text",
                        text: `Email sent successfully!\n\nTo: ${args.to}\nSubject: ${args.subject}\nMessage ID: ${messageId}`,
                    }],
                };
            } catch (error) {
                logger.error("[google-gmail] send error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-gmail-search
    server.tool(
        "google-gmail-search",
        "Search emails in Gmail using Gmail search syntax",
        {
            query: z.string().describe("Gmail search query (e.g., 'has:attachment from:user@example.com after:2024/01/01')"),
            max_results: z.number().optional().describe("Maximum number of results (default: 10)"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const messages = await gmailApi.searchEmails(accountName, args.query, args.max_results);

                if (messages.length === 0) {
                    return { content: [{ type: "text", text: "No messages found matching the query." }] };
                }

                const list = messages
                    .map((m, i) => `${i + 1}. ${m.subject}\n   From: ${m.from}\n   Date: ${m.date}\n   Snippet: ${m.snippet}\n   ID: ${m.id}`)
                    .join("\n\n");

                return {
                    content: [{ type: "text", text: `Found ${messages.length} message(s):\n\n${list}` }],
                };
            } catch (error) {
                logger.error("[google-gmail] search error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // ── Slides Tools ────────────────────────────────

    // Tool: google-slides-get-presentation
    server.tool(
        "google-slides-get-presentation",
        "Get the content of a Google Slides presentation (extracts text from all slides)",
        {
            presentation_id: z.string().describe("The ID of the presentation"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const result = await slidesApi.getPresentation(accountName, args.presentation_id);

                const slideTexts = result.slides
                    .map((s) => `--- Slide ${s.slideIndex} ---\n${s.textContent || "(empty slide)"}`)
                    .join("\n\n");

                return {
                    content: [{
                        type: "text",
                        text: `Presentation: ${result.title}\nSlides: ${result.slides.length}\n\n${slideTexts}`,
                    }],
                };
            } catch (error) {
                logger.error("[google-slides] get-presentation error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );

    // Tool: google-slides-list-slides
    server.tool(
        "google-slides-list-slides",
        "List all slides in a Google Slides presentation with their text content",
        {
            presentation_id: z.string().describe("The ID of the presentation"),
            account: accountParam,
        },
        async (args, extra) => {
            try {
                const accountName = await resolveAccountWithAcl(extra.sessionId, args.account);
                const slidesList = await slidesApi.listSlides(accountName, args.presentation_id);

                if (slidesList.length === 0) {
                    return { content: [{ type: "text", text: "No slides found in the presentation." }] };
                }

                const list = slidesList
                    .map((s) => `Slide ${s.slideIndex} (ID: ${s.objectId})\n   Preview: ${s.textContent.substring(0, 100)}${s.textContent.length > 100 ? "..." : ""}`)
                    .join("\n\n");

                return {
                    content: [{ type: "text", text: `Found ${slidesList.length} slide(s):\n\n${list}` }],
                };
            } catch (error) {
                logger.error("[google-slides] list-slides error:", error);
                return {
                    content: [{ type: "text", text: `Error: ${error instanceof Error ? error.message : String(error)}` }],
                    isError: true,
                };
            }
        }
    );
}